...
{
    public partial class Ad_Winner_Edit : System.Web.UI.Page
    {
        public GetRole mGetRole;
        Winner mWinner = new Winner();
        int EditID = 0;

        public string ParentPath = "../Admin_Service/Ad_Winner.aspx";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!IsPostBack)
                {
                    ...
                    if (EditID > 0)
                    {
                        DataTable mTable = mWinner.Select(1, EditID.ToString());
        ...
        private void Save(bool IsApply)
        {
            ...
            {              
                ...
                DataSet mSet = mWinner.CreateDataSet();
                AddNewRow(ref mSet);
                //Nếu là Edit
                if (EditID > 0)
                {
                    if (mWinner.Update(1, mSet.GetXml()))
                    {
                        #region Log member
                        MemberLog mLog = new MemberLog();
                        MemberLog.ActionType Action = MemberLog.ActionType.Update;
                        mLog.Insert("Winner", ViewState["OldData"].ToString(), mSet.GetXml(), Action, true, string.Empty);
                        #endregion
                ...
                {
                    if (mWinner.Insert(0, mSet.GetXml()))
                    {
                        #region Log member
                        MemberLog mLog = new MemberLog();
                        MemberLog.ActionType Action = MemberLog.ActionType.Insert;
                        mLog.Insert("Winner", string.Empty, mSet.GetXml(), Action, true, string.Empty);
                        #endregion

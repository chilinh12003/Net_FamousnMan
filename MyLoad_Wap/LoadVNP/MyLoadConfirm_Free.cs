...
{
    public class MyLoadConfirm_Free : MyLoadBase
    {
       
        public string MSISDN = string.Empty;
        public string Keyword = string.Empty;
        public int PartnerID = 0;
        public MyLoadConfirm_Free(string MSISDN, string Keyword, int PartnerID)
        {

            if (GetCurrentDeviceType == MyConfig.DeviceType.Tablet)
            {
                mTemplatePath = "~/Templates/LandingPage_2/Confirm_Free.htm";
            }
            else
            {
                mTemplatePath = "~/Templates/LandingPage/Confirm_Free.htm";
            }
            this.MSISDN = MSISDN;

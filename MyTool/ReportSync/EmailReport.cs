    ...
    public class EmailReport
    {
        ...
        public DataTable GetDataFromDBByDay(DateTime ReportDay)
        {
            try
            {
                ...
                //Lấy đăng ký mới
                DataTable mTable_Reg = mReportCharge.Select(2, ReportDay.ToString(FormatDay_DB), ((int)ChargeLog.ChargeType.REG).ToString(), ((int)ChargeLog.ChargeStatus.ChargeSuccess).ToString());
                ...
                DataTable mTable_Dereg = mReportCharge.Select(2, ReportDay.ToString(FormatDay_DB), ((int)ChargeLog.ChargeType.UNREG).ToString(), ((int)ChargeLog.ChargeStatus.ChargeSuccess).ToString());
                if (mTable_Dereg.Rows.Count > 0)
                {
                    int.TryParse(mTable_Dereg.Compute("SUM(TotalCount)", string.Empty).ToString(), out NewDereg);
                }
                //Charge thành công
                DataTable mTable_Renew = mReportCharge.Select(2, ReportDay.ToString(FormatDay_DB), ((int)ChargeLog.ChargeType.RENEW).ToString(), ((int)ChargeLog.ChargeStatus.ChargeSuccess).ToString());
